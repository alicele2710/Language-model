...
class Attention:
        ...
        def backward(self, dLdXnew):
            ...
            dLdA_sig       = torch.matmul(dLdXnew, self.V.transpose(-2, -1))# TODO
            dLdA_w         = 1/(self.K.size(-1) ** 0.5)*self.softmax.backward(dLdA_sig)# TODO

            # Derivatives wrt keys, queries, and value
            
            self.dLdV      = torch.matmul(self.A_sig.transpose(-2, -1), dLdXnew)# TODO
            self.dLdK      = torch.matmul(dLdA_w.transpose(-2, -1), self.Q)# TODO
            ...
            self.dLdWq     = torch.matmul(self.X.transpose(-2, -1), self.dLdQ).sum(dim=0)# TODO
            self.dLdWv     = torch.matmul(self.X.transpose(-2, -1), self.dLdV).sum(dim=0)# TODO
            self.dLdWk     = torch.matmul(self.X.transpose(-2, -1), self.dLdK).sum(dim=0)# TODO

            # Derivative wrt input

            dLdX = (
    torch.matmul(self.dLdV , self.W_v.transpose(-2, -1)) +
    torch.matmul(self.dLdQ, self.W_q.transpose(-2, -1)) +
    torch.matmul(self.dLdK, self.W_k.transpose(-2, -1))
)
